...
{
	auto enemyStartingArea = m_pBattleMapSystem->getEnemyStartingArea();
	for (auto iter : m_vCompterShips)
	{
		int randX = RANDOM_MAKER->GetInt(-200, 200);
		int randY = RANDOM_MAKER->GetInt(-200, 200);
		iter->setCurrentCenterX(enemyStartingArea->getCenterX() + randX);
		iter->setCurrentCenterY(enemyStartingArea->getCenterY() + randY);
