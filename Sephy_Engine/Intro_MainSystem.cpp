...
bool CIntro_MainSystem::initialize(Game * gamePtr)
{
	...
	try
	{
		...
		m_pJoinButton->initialize(m_pGraphics, m_pInput,
			...
			(g_fScreenHeight / 2) - (m_pJoinButton->getHeight() / 2) + introMainSystemNS::JOIN_BUTTON_RELATE_Y, "Join", true);
		...
		m_pCancelButton->initialize(m_pGraphics, m_pInput,
			...
			(g_fScreenHeight / 2) - (m_pCancelButton->getHeight() / 2) + introMainSystemNS::CANCEL_BUTTON_RELATE_Y, "Cancel", true);
